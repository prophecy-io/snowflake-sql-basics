...
{%- if split_strategy == 'splitColumns' -%}
    ...
    select
        ...
        {%- for i in range(1, noOfColumns) %}
            TRIM(s.tokens[{{ i - 1 }}],'"') as {{ outputRootName }}_{{ i }}_{{ 'generated' }}{% if not loop.last or leaveExtraCharLastCol %}, {% endif %}
        {%- endfor %}
        ...
        {%- else %}
            s.tokens[{{ noOfColumns - 1 }}] as {{ outputRootName }}_{{ noOfColumns }}_{{ 'generated' }}
        {%- endif %}
    from source as s
    )
...
{%- elif split_strategy == 'splitRows' -%}
    SELECT {{ relation_name }}.*,TRIM(REGEXP_REPLACE(s.value, '[{}_]', ' ')) AS {{ columnName }}_{{ 'generated' }}
